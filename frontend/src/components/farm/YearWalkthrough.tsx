...
const catchmentYearLevel = (
  nLoadKg: number,
  quotaKgN: number,
  yearFieldCount: number,
  expectedFieldCount: number,
): QuotaStatusLevel => {
  if (yearFieldCount === 0) return 'uncalculated'
  const level = quotaStatusLevel(nLoadKg, quotaKgN, true)
  if (level === 'over' || level === 'noData') return level
  return yearFieldCount < expectedFieldCount ? 'partial' : level
}

const barWidthClass = (count: number): string =>
  count > 6 ? 'w-1.5' : count > 4 ? 'w-2.5' : count > 3 ? 'w-3' : 'w-4'

const describeYearQuota = (
  quotaCatchmentCount: number,
  overCount: number,
): string => {
  if (quotaCatchmentCount === 0) return ''
  if (quotaCatchmentCount === 1) {
    return overCount > 0 ? ', over kvoten' : ', under kvoten'
  }
  return overCount > 0
    ...
    catchments.map((catchment) => {
      if (entry === null) {
        return {
          ...
          quotaKgN: catchment.totals.nLoadQuotaKgN,
          level: catchment.level,
        }
      }
      const yearTotals = catchmentTotalsByYear?.get(catchment.catchmentId)?.[
        entry.year
      ]
      const nLoadKg = yearTotals?.nLoadKg ?? 0
      const quotaKgN = yearTotals?.quotaKgN ?? 0
      return {
        catchment,
        nLoadKg,
        quotaKgN,
        level: catchmentYearLevel(
          nLoadKg,
          quotaKgN,
          yearTotals?.fieldCount ?? 0,
          catchment.totals.fieldCount,
        ),
      }
    })

  const countFieldsWithYearValues = (year: number): number => {
    let count = 0
    catchmentTotalsByYear?.forEach((byYear) => {
      count += byYear[year]?.fieldCount ?? 0
    })
    return count
  }
