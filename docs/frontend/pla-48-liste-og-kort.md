...
- Valg, række til kort: klik på række sætter `selectedFieldId`; kortet fremhæver via de eksisterende `selected-farm-field`-lag og zoomer ind på marken (`fitBounds`, padding 64, maxZoom 16, 500 ms). Det gælder uanset om marken vælges fra listen, kortet, Find mark eller forrige/næste i markpanelet. Fjernes valget, zoomer kortet ud til alle bedriftens marker som Vis alle (10-09, Nikolais ønske; før panorerede kortet kun, hvis marken var uden for udsnittet). Dobbeltklik på række gør det samme som klik.
...
- Kystvandopland-chips: klik fremhæver oplandet (`aria-pressed`, ring): rækker uden for oplandet dæmpes til `opacity-50` uden at filtrere, så tabelfodens totaler står fast, oplandets marker lægges øverst i listen i deres hidtidige rækkefølge (10-09, Nikolais ønske), og listen scroller til toppen; kortet dæmper polygoner uden for oplandet (fill-opacity 0,12) og laver `fitBounds` til oplandets marker, og zoomer tilbage til alle bedriftens marker (som Vis alle), når fremhævningen fjernes. Klik igen eller Escape rydder. Escape-rækkefølge: markpanel, derefter opland, år kun med fokus i årsgennemgangens radiogroup.
