...
def evaluate_real_history_for_field(
    ...
    for i in range(8):
        ...
        f1 = afgroede_normer.lookup_nfix(prev_code, jbnr, irrigated) if prev_code is not None else 0.0
        f2 = afgroede_normer.lookup_nfix(prev2_code, jbnr, irrigated) if prev2_code is not None else 0.0

        norm = afgroede_normer.lookup_norm(this_code, jbnr, irrigated) if this_code is not None else None
        prev_norm = afgroede_normer.lookup_norm(prev_code, jbnr, irrigated) if prev_code is not None else None
        ...
        years.append(RotationCandidateYearResult(
            year=RotationYear(
                ...
                afgrode_navn=crop_params.get("navn", "Ukendt") if this_code is not None else "Ukendt",
            ),
