...
```
pixi run db-migrate
pixi run load-registry-2026   # runs the complete sequence below
...
rækkefølge efter den. `load-registry-2026` er den anbefalede vej, så
udledningsgrænser og de afledte markkvoter ikke ved en fejl efterlades som 0.
...
En kørsel nulstiller først tidligere overlay-værdier, opdaterer registeret og
synkroniserer derefter grænse/kvote til gemte marker med et matchende `imk_id`.
